...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
    ...
    const embed = new EmbedBuilder()
        .setTitle(isTimeout ? "🔇 Thành viên bị Timeout" : "🔊 Timeout được gỡ")
        ...
        .addFields(
            ...
            {
                name: "🛡️ Người thực hiện",
                value: executor ? `<@${executor.id}>` : "Không xác định"
            }
        )
        .setTimestamp();
...
{
    name: "🛡️ Người thực hiện",
    value: entry?.executor ? `<@${entry.executor.id}>` : "Không xác định"
}
)
client.on("roleUpdate", async (oldRole, newRole) => {
    ...
    const embed = new EmbedBuilder()
        .setTitle("🎭 Vai trò được chỉnh sửa")
        .addFields(
            {
                name: "📌 Role",
                value: newRole.name
...
client.on("roleUpdate", async (oldRole, newRole) => {
    ...
    const embed = new EmbedBuilder()
        .setTitle("⚙️ Quyền Role được chỉnh sửa")
        .addFields(
            {
                name: "🎭 Role",
...
client.on("channelUpdate", async (oldChannel, newChannel) => {
    ...
    if (
        JSON.stringify(oldChannel.permissionOverwrites.cache.map(x => x.allow.bitfield)) ===
        JSON.stringify(newChannel.permissionOverwrites.cache.map(x => x.allow.bitfield))
    ) return;
    ...
    const embed = new EmbedBuilder()
        .setTitle("⚙️ Quyền Kênh được chỉnh sửa")
        .addFields(
            {
                name: "📂 Kênh",
                value: `${newChannel}`
